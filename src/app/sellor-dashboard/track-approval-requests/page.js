...
const page = ({params}) => {
  ...
  useEffect(()=>{ 
    ...
    if(globalData.sellor){
      ...
      fetch(`${baseUrl}api/sellor/get-brand-list?user_id=${globalData.sellor._id}&status=${status?status:''}&search=${search?search:""}`,{
        method:"GET", 
      }).then((response)=>{
          if(!response.ok){
  ...
  function searchBrand(e){
    e.preventDefault();
    const link = `${baseUrl}sellor-dashboard/track-approval-requests?status=&search=${searchText}`
    // setSearchText("")
    router.push(link) 
  }
  ...
  function deletdBrand(e,_id){
    ...
    if(globalData.sellor){
      $('.loaderouter').css('display','flex') 
      fetch(`${baseUrl}api/sellor/get-brand-list`,{
        method:"DELETE", 
            ...
            <div className="col-lg-8 col-md-8 col-sm-6 col-xs-12">
              <div className="breadcome-heading">
                <form role="search" className="sr-input-func"  onSubmit={(e)=>searchBrand(e)}>
                  <input type="text" placeholder="Search your Approval by brand name, resuest id" className="search-int form-control" 
            ...
            <tr className="winner__table">
              <th colSpan={10}>
                <div className="table_menu">
                  <ul>
                    <li><Link href={`${baseUrl}sellor-dashboard/track-approval-requests?status=`} className={`${status=="all" || !status ?"active":''}`} >All Request({total.totalBrand})</Link></li>
                    <li><Link href="#">Action Required(0)</Link></li>
                    <li><Link href={`${baseUrl}sellor-dashboard/track-approval-requests?status=${2}`} className={`${status==2?"active":''}`} > Pending({total.totalPendingBrand})</Link></li>
                    <li><Link href={`${baseUrl}sellor-dashboard/track-approval-requests?status=${1}`} className={`${status==1?"active":''}`} > Approved({total.totalApproveBrand})</Link></li>
                  </ul>
                </div>
              </th>
            </tr>

...
function page() {
  ...
  useEffect(()=>{ 
    if(globalData.sellor){
      ...
      fetch(`${baseUrl}api/sellor/get-profile?user_id=${globalData.sellor._id}&with_data=pickupAddress`,{
        method:"GET", 
      }).then((response)=>{
          if(!response.ok){
...
function submitUpdateForm(e){
  ...
  $('.loaderouter').css('display','flex');
  fetch(`${baseUrl}api/sellor/update-profile?update=pickUpAddress`,{
    method:"POST",
    body:JSON.stringify({
      ...sellor,
      address:addressData
    ...
    $('.loaderouter').css('display','none') 
      if(res.status){
        ...
        router.push('/sellor/al/return-address')

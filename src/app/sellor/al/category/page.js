...
function page() {
  ...
  useEffect(()=>{ 
    if(globalData.sellor){
      ...
      fetch(`${baseUrl}api/sellor/get-profile?user_id=${globalData.sellor._id}&category=Yes`,{
        method:"GET", 
      }).then((response)=>{
          if(!response.ok){
...
function submitUpdateForm(e){
  e.preventDefault();
  $('.loaderouter').css('display','flex');
  fetch(`${baseUrl}api/sellor/update-profile?update=contact_details`,{
    method:"POST",
    ...
    $('.loaderouter').css('display','none') 
      if(res.status){
        ...
        router.push('/sellor/al/display-information')
      }
  })

...
const page = () => {
  ...
  useEffect(() => {
    if (globalData.sellor) {
      ...
      fetch(`${baseUrl}api/sellor/get-profile?user_id=${globalData.sellor._id}&with_data=taxInformation`, {
        method: "GET",
      }).then((response) => {
        if (!response.ok) {
  ...
  function submitUpdateForm(e) {
    ...
    fetch(`${baseUrl}api/sellor/update-profile?update=taxInformation`, {
      method: "POST",
      body: formData
    }).then((response) => {
      if (!response.ok) {
      ...
      if (res.status) {
        ...
        router.push('/sellor/al/shipping-setting')

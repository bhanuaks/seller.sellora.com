...
export async function POST(request) {
    ...
    await connectDb()
    const {name, mobile, email, password, otp, mobile_code, country_s_name} = await request.json();
    ...
    try{
        ...
        if(sellerEmailExists){
            ...
            if(otpData && otpData.otp_for == "register" && otpData.otp == otp){
                ...
                return responseFun(true, {message:"Your acoount created successfully"}, 200) 
            }else{

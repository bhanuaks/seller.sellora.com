...
async function updateContactDetails(request){
    const {_id, name, email, mobile, country_s_name, mobile_code } = await request.json();
    ...
    try{
        ...
        const checkMobile = await sellerModel.findOne({ 
            _id: { $ne: new mongoose.Types.ObjectId(_id) }, 
            mobile: mobile  
        });
        ...
        const sellerUpdate = await sellerModel.findByIdAndUpdate(_id,{
            ...
            email,
            mobile, 
            country_s_name:country_s_name || seller.country_s_name,
            mobile_code: mobile_code || seller.mobile_code,

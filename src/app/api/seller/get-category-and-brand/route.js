...
export async function POST(request) { 
    await connectDb()
    const {seller_id, category_id, subcategory_id, childcategory_id, brand_id, seller, product_id,  withData, required_variant } = await request.json();
    ...
    try{
        ...
        if(subcategory_id){
             subcategory = await subCategory.findById(subcategory_id).select("_id subCategoryName slug category_id")
        }
        let childcategory = null;
        if(childcategory_id){ 
             childcategory = await  ChildCategory.findById(childcategory_id).select("_id subCategoryName slug")
        }
        ...
        if(product_id && required_variant == "yes"){
        ...
        }else if(product_id && withData == "Compliance"){
            product = await productModel.findById(product_id);
            compliance = await productOtherDetailModel.findOne({product_id: new mongoose.Types.ObjectId(product_id)});
        }else if(product_id){
            product = await productModel.findById(product_id);
